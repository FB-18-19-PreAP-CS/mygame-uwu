...
def beatmap():
    ...
    for space_a,space_s,space_d,space_f in zip(column_a,column_s,column_d,column_f):
        ...
        if space_a == "-":
            blank_a = Blank([400,20 - row * spacing])
        
            buttons_a.append(blank_a)
            
        if space_a == "x":
            butt_a = Button([400,20 - row * spacing])
            
            buttons_a.append(butt_a)
            
        if space_s == "-":
            blank_s = Blank([450,20- row * spacing])
            
            buttons_s.append(blank_s)


        if space_s == "x":
            butt_s = Button([450,20- row * spacing])
            
            buttons_s.append(butt_s)


        if space_d == "-":
            blank_d = Blank([500,20- row * spacing])
            
            buttons_d.append(blank_d)

        if space_d == "x":
            butt_d = Button([500,20- row * spacing])
            
            buttons_d.append(butt_d)

        if space_f == "-":
            blank_f = Blank([550,20- row * spacing])
            
            buttons_f.append(blank_f)

        if space_f == "x":
            butt_f = Button([550,20- row * spacing])
